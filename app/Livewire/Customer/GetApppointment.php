...
class GetApppointment extends Component
{
    ...
    public function render()
    {
        return view('livewire.customer.get-apppointment', [
            ...
            'feedbacks'    => Feedback::whereHas('appointment', function ($query) {
                $query->where('shop_id', $this->shop_id);
            })->inRandomOrder()->take(3)->get(),
            'appointments' => Appointment::where('shop_id', $this->shop_id)->where('barber_id', $this->barber_id)->where('status', 'pending')->get(),
        ]);
    }
    ...
    public function previewRecord()
    {
        ...
        $exists = Appointment::where('shop_id', $this->shop_id)->where('barber_id', $this->barber_id)->where('status', 'pending')
            ->exists();

        if ($exists) {
            noty()->error('Date has already been taken');
            return; // Stop execution if the date is already taken
        }

        // If the date is available, proceed to show the preview modal
